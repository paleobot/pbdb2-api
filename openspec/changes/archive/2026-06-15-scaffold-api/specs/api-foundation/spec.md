...
versions can be introduced without modifying existing routes.
...
#### Scenario: Paths outside the version prefix are not served

- **WHEN** a client requests a resource path that omits the `/api/v1` prefix
