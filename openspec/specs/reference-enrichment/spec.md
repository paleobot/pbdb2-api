...
declaration rather than by new query code. The first targets are `references`
and `authorities`: `authorities` carry a single `reference`; `collections` and
`schemas` carry a `primaryReference` and an `additionalReferences` array.
`specimens` follows the same primary + additional pattern once it has a backing
table.
